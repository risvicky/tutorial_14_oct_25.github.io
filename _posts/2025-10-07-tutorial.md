...
Then, we will create a box with a size of 3.5 nm x 3.5 nm x 3.5 nm by copying and pasting the following lines into `empty.gro`.
   ```
    Cubic box
    0
    3.50000   3.50000   3.50000
   ```
The first line, `Cubic box`, is a comment line; the second line, `0`, indicates that there are no atoms in the box; and the third line specifies the dimensions of the box in nanometers, 3.5 nm in each direction (x, y, z). This ***.gro*** is written in [Gromos87](https://manual.gromacs.org/archive/5.0.4/online/gro.html) format. 
You can also download the [empty.gro]({{ '/file/empty.gro' | relative_url }}) (right click to download) and save it in the same folder.
...
You can also download the [no3.gro]({{ '/file/no3.gro' | relative_url }}) (right click to download) and save it in the same folder.
...
If the insertions were successful, the output printed in the terminal should look like this.
```
...
1
   ...
   1.00000   1.00000   1.00000
   ```
You can also download the [na.gro]({{ '/file/na.gro' | relative_url }}) (right click to download) and save it in the same folder.
...
<div class="exercise-box">
  ...
  <p>
    Use the `insert-molecules` command to add 10 Na<sup>+</sup> ions to the same `conf.gro` file with radius of 0.5 nm from each other.
  </p>
...
Download the [h2o.gro]({{ '/file/h2o.gro' | relative_url }}) template for H<sub>2</sub>O molecule and save it in the same folder.
...
Notes: Since we want to insert 10 Na<sup>+</sup> ions and 800 water molecules into the same box, the input and output files should both be called conf.gro.
...
If you are unable to insert the molecules, you can download the [conf.gro]({{ '/file/conf.gro' | relative_url }}) file (right click to download) to continue the tutorial.
...
<p align="center">
  <img src="{{ '/figure/nano3.png' | relative_url }}" alt="nano3" width="500">
</p>
...
The [topol.gro]({{ '/file/topol.gro' | relative_url }}) must be in the same folder as the **conf.gro** file. The **topol.top** file contains:
```
...
- [forcefield.itp]({{ '/file/forcefield.itp' | relative_url }})
- [h2o.itp]({{ '/file/h2o.itp' | relative_url }})
- [na.itp]({{ '/file/na.itp' | relative_url }})
- [no3.itp]({{ '/file/no3.itp' | relative_url }})
...
nstxout = 10
```
You can also download the [min.mdp]({{ '/file/min.mdp' | relative_url }}) (right click to download).
...
The trajectory is generated in a **.trr** file that can be visualised using VMD.
...
Too many warnings (1).
```
rerun the command and add `-maxwarn 1`.
 ```bash
gmx grompp -f min.mdp -c conf.gro -o min -pp min -po min -maxwarn 1
```
...
```
<p align="center">
  <img src="{{ '/figure/no3_h2o.png' | relative_url }}" alt="o3_ion" width="auto">
</p>

<p align="center">
  <img src="{{ '/figure/na.png' | relative_url }}" alt="na_ion" width="auto">
</p>
...
<p align="center">
  <img src="{{ '/figure/min_pe.png' | relative_url }}" alt="min_pe" width="600">
</p>
...
; Integrator, timestep, and total run time
...
You can also download the [nvt.mdp]({{ '/file/nvt.mdp' | relative_url }}) (right click to download).
...
<p align="center">
  <img src="{{ '/figure/nvt-t.png' | relative_url }}" alt="nvt-t" width="400">
</p>
...
```
You can also download the [npt.mdp]({{ '/file/npt.mdp' | relative_url }}) (right click to download).
...
<p align="center">
  <img src="{{ '/figure/npt_T.png' | relative_url }}" alt="npt_T" width="500">
</p>

<p align="center">
  <img src="{{ '/figure/npt_P.png' | relative_url }}" alt="npt_P" width="500">
</p>

<p align="center">
  <img src="{{ '/figure/npt_rho.png' | relative_url }}" alt="npt_rho" width="500">
</p>


The results show that the temperature, pressure and density are maintained well during *NPT* run by reaching 360 K, 1 bar and 1000 kg/m<sub>3</sub>, respectively. 

## 5. **Production Run**

For the production run, we will perform **400 ps** run in **NVT** ensemble and the trajectory printes every ps.

Create a new input script called **production.mdp** and copy the following lines into it.

```
integrator = md           ; A leap-frog algorithm for integrating Newton’s equations of motion.
nsteps = 400000            ; Simulation will run for 400000 steps
dt = 0.001                ; with timestep of 0.001  ps
...
ref-t = 360 360           ; the same T for two groupswith T of 360 K
```
All the commands are similar from the **nvt.mdp** file.

We will use the **npt.gro** from **NPT** ensemble as the starting configuration.

```
gmx grompp -f production.mdp -c npt.gro -o production -pp production -po production -maxwarn 1
...
Radial distribution function (RDF), g(r) can be used to describe the number of atoms in a shell $$ \Delta r $$ at a distance *r* from a central species/atom.

RDF between Na<sup>+</sup> ions and H<sub>2</sub>O molecules can be computed using `gmx rdf` command.

```
gmx rdf -f production.xtc -s production.tpr -o rdf-na-h2o.xvg
```
choose sodium ions as *reference* by typing `3` and water as *selection* by typing `4` and press `Ctrl+D`.

For RDF of NO<sub>3</sub><sup>-</sup> ions with water molecules can be done using command.
```
gmx rdf -f production.xtc -s production.tpr -o rdf-no3-h2o.xvg
```
and choose `2` and `4`.

<p align="center">
  <img src="{{ '/figure/rdf-na-no3.png' | relative_url }}" alt="rdf_na_no3" width="500">
</p>

The RDFs of NO<sub>3</sub><sup>-</sup> and H<sub>2</sub>O have several peaks because of the inclusion of all atoms from H<sub>2</sub>O including hydrogen.
We will separate the RDFs by including only water oxygen atoms (with name `OW1`) and nitrogen atom of NO<sub>3</sub><sup>-</sup>.

We can create groups using `gmx make_ndx` command as follows.

```
gmx make_ndx -f production.tpr
```
Then type `a OW1`, press enter, `a N1` press enter and `q` to save and quit. This step will create a file **index.ndx**

The rerun `gmx rdf` command using **index.ndx** and select the new groups.
For Na<sup>+</sup> ions to `OW1`:

```
gmx rdf -f production.xtc -s production.tpr -o rdf-na-OW1.xvg -n index.ndx
```
select 3 and 7, respectively.

For NO<sub>3</sub><sup>-</sup> ions to `OW1`:

```
gmx rdf -f production.xtc -s production.tpr -o rdf-no3-OW1.xvg -n index.ndx
```
select 8 and 7, respectively.

<p align="center">
  <img src="{{ '/figure/na-ow1.png' | relative_url }}" alt="rdf-na-no3" width="500">
</p>
...
<p align="center">
  <img src="{{ '/figure/qr.png' | relative_url }}" alt="qr" width="500">
</p>


References:
1. GROMACS User Manual, https://manual.gromacs.org/current/install-guide/index.html
2. https://gromacstutorials.github.io/sphinx/build/html/tutorials/tutorial1/bulk-solution.html